...
====
WARNING: Damit das Erzeugen eines Release problemlos funktioniert, müssen
folgende Voraussetzungen erfüllt sein:
====

====
WARNING: 1. Die SSH-Konfiguration muss korrekt aufgesetzt sein. Aufzuführen, was das in Einzelnen bedeutet, führt an dieser Stelle zu weit. Der Hinweis mag vorerst genügen.
====

====
WARNING: 2. Die beiden SSH-Schlüsseldateien `id_rsa.kvk@syniphos` und `id_rsa.kvk@syniphos.pub` müssen im Verzeichnis `.\ADHelper\misc\build-res\` vorliegen. 
Da es sich bei diesen Dateien um Schlüssel handelt, die Zugriff auf den Server gewähren, sind sie nicht im Repository gespeichert! Die Dateien werden auf Anfrage bereit gestellt.
====
